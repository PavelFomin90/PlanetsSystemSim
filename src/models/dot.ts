...
class Dot implements IDot {
    ...
    move() {
        ...
        this.coords.x = coords.x + velocity.length*Math.cos(toRadians(velocity.angle));
        this.coords.y = coords.y + velocity.length*Math.sin(toRadians(velocity.angle));
    }
    ...
    computeGravityForce(planets: IDot[]) {
        ...
        planets.forEach((planet, _index) => {
            if (planet !== this) {
                const distance = Math.sqrt(Math.pow(x - planet.coords.x, 2) + Math.pow(y - planet.coords.y, 2));
                const length = GRAVCONST*(planet.mass) / Math.pow(distance, 2);
                const angle = toDegrees(Math.atan2(planet.coords.y - y, planet.coords.x - x));
